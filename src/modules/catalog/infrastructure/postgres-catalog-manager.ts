...
export class PostgresCatalogManager implements CatalogManager {
  ...
  async list(after?: string) {
    ...
    const products = rows.slice(0, CATALOG_MANAGEMENT_PAGE_SIZE).map((r): ManagedProduct => {
      const command = catalogSaveSchema.parse({ id: r.id, expectedVersion: Number(r.version), requestId: r.id, slug: r.slug, name: r.name,
        subtitle: r.subtitle, description: r.description, price: Number(r.price_minor), imageUrl: r.image_url, imageAlt: r.image_alt,
      ...
      return { id: command.id, version: command.expectedVersion, slug: command.slug, name: command.name, subtitle: command.subtitle,
        description: command.description, price: command.price, imageUrl: command.imageUrl, imageAlt: command.imageAlt, palette: command.palette,
  ...
  async save(input: CatalogSave, actor: ManagementActor) {
    ...
    const [prior] = await this.tx`SELECT command = ${this.tx.json(p)} AS matches FROM bloombox.catalog_changes
    ...
    if (!current) {
      ...
      await this.tx`INSERT INTO bloombox.catalog_products (id, slug, status, available, name, subtitle, description, price_minor,
        image_url, image_alt, palette, occasions, flowers, grower)
        VALUES (${p.id}, ${p.slug}, ${p.status}, ${p.available}, ${p.name}, ${p.subtitle}, ${p.description}, ${p.price},
      ...
      await this.tx`UPDATE bloombox.catalog_products SET slug=${p.slug}, status=${p.status}, available=${p.available}, name=${p.name},
        subtitle=${p.subtitle}, description=${p.description}, price_minor=${p.price}, image_url=${p.imageUrl}, image_alt=${p.imageAlt},
