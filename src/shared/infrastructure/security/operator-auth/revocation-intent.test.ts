...
const target = { shop: "example.myshopify.com", permissionId: "00000000-0000-4000-8000-000000000002", reviewedVersion: 3 };
const service = () => new OperatorRevocationIntent(secret, origin, () => now);
describe("encrypted permission revocation context", () => {
  ...
  it("rejects tampering, other actors/logins, mode, origin, secret and oversized input", async () => {
    ...
    const calls = [service().read(parts.join("."), actor, true), service().read("x".repeat(2049), actor, true),
      service().read(token, { ...actor, operatorId: target.permissionId }, true),
      service().read(token, { ...actor, expiresAt: new Date(actor.expiresAt.getTime() + 1000) }, true),
      service().read(token, actor, false),
      new OperatorRevocationIntent("another-synthetic-secret", origin, () => now).read(token, actor, true),
      new OperatorRevocationIntent(secret, "https://another.example", () => now).read(token, actor, true)];
    for (const result of await Promise.allSettled(calls)) {
      expect(result.status).toBe("rejected");
      if (result.status === "rejected") expect(result.reason).toEqual(new PermissionRevocationError("REVIEW_REQUIRED"));
    }
  });
  it("expires after five minutes or the original session, whichever is earlier", async () => {
    const token = await service().issue(target, actor, true);
    await expect(new OperatorRevocationIntent(secret, origin, () => new Date(now.getTime() + 300000)).read(token, actor, true))
      .rejects.toEqual(new PermissionRevocationError("REVIEW_REQUIRED"));
    const shortActor = { ...actor, expiresAt: new Date(now.getTime() + 10000) };
    const shortToken = await service().issue(target, shortActor, true);
    await expect(new OperatorRevocationIntent(secret, origin, () => shortActor.expiresAt).read(shortToken, shortActor, true))
      .rejects.toEqual(new PermissionRevocationError("REVIEW_REQUIRED"));
    await expect(service().issue(target, { ...actor, expiresAt: now }, true)).rejects.toEqual(new PermissionRevocationError("NOT_AUTHORIZED"));
  });
});
