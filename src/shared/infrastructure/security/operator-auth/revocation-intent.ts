...
import { z } from "zod";
import { PermissionRevocationError, type PermissionRevocationRequest } from "@/modules/fulfillment/public";

const audience = "bloombox:permission-revocation-intent:v1";
...
type Target = Omit<PermissionRevocationRequest, "idempotencyKey" | "reason">;

/** Encrypted review context, never management authority. The command revalidates current scoped manager rights. */
export class OperatorRevocationIntent {
  ...
  async issue(target: Target, actor: Actor, testMode: boolean): Promise<string> {
    ...
    if (!Number.isFinite(issued) || expires <= issued) throw new PermissionRevocationError("NOT_AUTHORIZED");
  ...
  async read(token: string, actor: Actor, testMode: boolean): Promise<Omit<PermissionRevocationRequest, "reason">> {
    try {
      ...
      return { shop: claims.shop, permissionId: claims.permissionId,
        reviewedVersion: claims.reviewedVersion, idempotencyKey: claims.jti };
    } catch { throw new PermissionRevocationError("REVIEW_REQUIRED"); }
  }
}
