...
export const googleAuthFetch: typeof fetch = async (input, init) => {
  ...
  if (url.protocol !== "https:" || url.username || url.password || !GOOGLE_AUTH_HOSTS.has(url.hostname) || (url.port && url.port !== "443")) {
    throw new Error("Google provider request rejected");
  }
  ...
  if (!reader) throw new Error("Google provider response unavailable");
  ...
  try {
    while (true) {
      ...
      if (length > RESPONSE_LIMIT) throw new Error("Google provider response too large");
      chunks.push(value);
    }
