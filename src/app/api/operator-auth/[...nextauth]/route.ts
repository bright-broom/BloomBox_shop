...
import { getOperatorAuth, isOperatorOrigin } from "@/shared/infrastructure/security/operator-auth/operator-auth";
...
async function handle(request: NextRequest) {
  try {
    ...
    if (!isOperatorOrigin(request, service.config)) return new Response(null, { status: 403, headers: privateHeaders });
    ...
    if (!allowed.some((action) => path === `/api/operator-auth${action}`)) return new Response(null, { status: 404, headers: privateHeaders });
    let bounded = request;
    ...
    console.error("operator_auth_route_unavailable");
