...
Migration 0020 protects owner-issued webhook retry audit receipts from runtime forgery and all update/delete operations. A second trigger prevents runtime writers from moving FAILED events to another status, while preserving ordinary claims/retries and payload retention. It grants no new runtime permission. The offline PLAN/APPLY tool requires `DATABASE_WEBHOOK_ADMIN_URL` with schema and Inbox table ownership, checks both guards, the target database/provider/account and review deadline, and atomically requeues an eligible FAILED event with an immutable receipt. Keep both triggers and audit history on rollback. Do not expose the owner credential to application/worker/preview environments. See [reviewed webhook recovery](../docs/operations/WEBHOOK_RETRY.md).

Migration 0021 adds an owner-backed, security-barrier Shopify diagnostic view exposing previously allowed queue metadata plus fixed last-failure categories. Apply updated roles.sql to grant SELECT on that view to bloombox_inbox_monitor; raw last_error_code remains denied. Unknown values never pass through. Diagnostics use the bounded view scan instead of joining/scanning queue history twice. No queue or commerce write authority is added. Deploy migration and role grant before the new diagnostic code; missing access fails closed. Rollback restores the prior diagnostic code before revoking the view grant and retains all recorded codes and migration history.
