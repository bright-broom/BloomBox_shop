...
DBトリガーは保存済み根拠の削除、確認時刻の後退、同一時刻の内容差替えを拒否します。受付・根拠・判定・監査・Outboxは原子的に保存し、最後の保存が失敗しても前段のOrder/Payment/Checkoutを維持します。取得失敗も以前の根拠を消しません。取得失敗そのものは新しい在庫観測として保存しません。追加の[担当者承認記録](SHOPIFY_FULFILLMENT_APPROVAL.md)は直近の成功した根拠を30秒以内に限って再判定しますが、直近の全照会の成功や現在の実在庫を保証するものではありません。実際の出荷には正式情報の再取得が必要です。
...
追加環境変数・依存パッケージ・DB権限はありません。戻す場合は在庫/発送受付の任意注入を外し、移行・根拠・監査を保持します。旧コードで確認結果を承認に流用しません。訂正は所有モジュールの前進変更で行います。店舗別の担当者権限・承認記録は内部実装済みです。次は担当者認証・確認画面、必要な拠点別制限、出荷直前の再確認、Shopify/倉庫での実行と結果不明時の照合が必要です。
