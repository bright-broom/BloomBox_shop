...
| Shopify通知と注文状態 | 署名検証・参照保存・重複排除は実装。Admin APIでの店舗/参照ID/金額検証も内部処理に実装（[仕様](SHOPIFY_ORDER_READS.md)）。購入試行照合も内部処理に実装（[仕様](SHOPIFY_ORDER_LINKS.md)）。残件は実店舗検証・保留運用、順序逆転、未着通知の再照合、支払待ち/入金済み/取消/返金の反映。ヘッダーや戻り画面だけで入金済みにしない。[詳細](SHOPIFY_WEBHOOKS.md) |
