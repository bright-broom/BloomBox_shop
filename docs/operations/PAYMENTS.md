...
- Shopify通知の署名検証・参照の暗号化保存・重複排除・店舗別Inboxを、初期状態で無効の受付として実装。[受付範囲と残件](SHOPIFY_WEBHOOKS.md)。
...
| Shopify通知と注文状態 | 署名検証・参照保存・重複排除は実装。Admin APIでの店舗/参照ID/金額検証も内部処理に実装（[仕様](SHOPIFY_ORDER_READS.md)）。購入試行照合も内部処理に実装（[仕様](SHOPIFY_ORDER_LINKS.md)）。決済状態の照合・監査付き保存も実装（[仕様](SHOPIFY_PAYMENT_EVIDENCE.md)）。残件は正式な注文受け入れ・状態表示、実店舗検証・保留運用、未着通知の再照合。ヘッダーや戻り画面だけで入金済みにしない。[詳細](SHOPIFY_WEBHOOKS.md) |
