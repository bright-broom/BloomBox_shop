...
1. 0023までのマイグレーションと `database/roles.sql` を適用します。旧アプリの在庫予約処理は変更せず共存できます。
...
- 1箱の送料は税込・JPYの整数で入力します。0円は送料無料、空欄は未設定です。未設定の商品は購入開始を拒否します。価格・送料は購入開始時に固定され、後の編集や再送で過去の金額は変わりません。初期条件は1回1箱です。[送料の設定と検証](PURCHASE_SHIPPING.md)を参照してください。
