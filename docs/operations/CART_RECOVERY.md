...
## 決済完了後のカートの片付け — 2026-09-14

本番経路では、Stripeで支払いを終えて完了画面に戻っても、ブラウザーのカートとヘッダーの件数が残っていました。顧客が「注文できていない」と誤解したり、同じ内容を編集したりする原因になります。

完了画面は、注文状況の問い合わせで注文の作成を確認できた場合だけ、その決済を始めたカートを空にします。照合には、購入準備のIDとカートの要求IDを使います。

- 決済処理中、支払い失敗、決済期限切れでは、注文がないためカートを残します。処理中の画面は自動で更新されるため、注文が確定した時点で片付けます。
- 別のカートが入っている場合、ストレージを読めない場合、処理中に別のカートへ置き換わった場合は、何も消しません。片付けに失敗しても、#166 の処理により同じカートから重複注文は始まりません。
- 注文状況に加えたのは、購入準備のIDと注文作成済みかどうかだけです。個人情報は増やしていません。購入準備のIDは、同じブラウザーのカートがすでに持っている値です。

検証：注文状況の問い合わせ、完了画面、片付け部品の単体試験を追加しました。完了画面はStripeの本番経路でのみ注文を表示するため、プレビューでの画面確認はしていません。

戻し方：この変更をrevertすると、支払い後もカートが残る状態に戻ります。DB、環境変数、保存形式v1は変わりません。
