...
NextAuth 5.0.0-beta.32 を固定採用しています。依存監査と合成プロバイダーテストは、実 Google の同意画面・本番コールバックを通した成功証跡の代用にはなりません。実接続、デプロイ先での Cookie/Origin、専用ロール設定、実店舗権限の失効を確認してから公開運用してください。事業条件、承認操作、Inbox 完了、本番 Gate、発送実行は引き続き別の確認・実装が必要です。
