...
- Listing rechecks each returned product's exact tag before mapping curated content. Structurally valid products outside the configured tag are excluded without stopping pagination; an entirely excluded result yields an empty catalog. Invalid response structure and invalid curated products still fail closed. This matches the exclusion behavior of handle and ID lookups.
