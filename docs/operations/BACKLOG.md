...
| 在庫の取引処理 | 予約・確定・安全な解放、同時購入、重複、通信断、保存失敗をDBで検証 | 顧客取消は2026-09-14にカート削除（単体・隔離DB試験）と、カートの内容変更・入替の保存（単体試験）へ接続。実Stripe、期限後復旧、発送・返品が残る。[在庫](NATIVE_INVENTORY.md) |
...
| P0-05 | 認証済みの実購入経路 | 内部実装済み・接続/検証待ち。本番はコードで停止。2026-09-14：未決済購入の顧客取消をカート削除へ接続（決済先選択前は即時解放、Session発行済みはStripe失効を確認し検証済み通知で解放）。単体・隔離DB試験のみで実Stripe未確認。同日、カートの内容変更・入替の保存でも前の購入準備を先に取り消すよう接続（単体試験）。終了済み・支払手続き完了済みの購入準備に紐付いたカートは、想定外エラーではなく要求IDの更新またはカートを空にして案内する（単体試験）。決済完了画面で注文の作成を確認したら、その決済を始めたカートを空にする（単体試験）。[ADR 0010追補](../architecture/adr/0010-native-inventory-reservations.md) | 本番受付を開けず隔離環境でGoogle→実商品相当DB→予約→Stripeテスト→署名通知→本人注文表示を接続。顧客取消は実Stripeテストで失効要求→期限切れ通知→在庫解放→カート非表示を確認する。[STRIPE](STRIPE.md)、[紐付け](PURCHASE_CUSTOMER_BINDING.md) |
| P0-06 | 在庫と決済の実整合 | 予約・確定・解放は実装/DB検証済み。判断・実接続待ち。2026-09-14：決済作成の応答が失われて決済IDを保存できなかった取引を、同じ購入準備IDの署名検証済み期限切れイベントで解放（DB試験追加、CIで確認）。同日、要求自体がStripeに届かなかった取引も、期限の15分後にワーカーがStripeのCheckout Sessionを一覧して照合し、該当なし・すべて期限切れ未払いの場合だけ解放（それ以外は解放せず要確認として監査に記録しワーカー失敗。DB試験追加、CIで確認）。要確認の調査手順は未整備 | 実在庫登録、24時間の予約保持・受付上限の業務確認、同時購入・失敗・取消・期限切れ・通信断・返金を確認。結果不明の取引を期限後に照合して復旧し、タイマーだけで予約を戻さない。[在庫](NATIVE_INVENTORY.md) |
...
| P0-19 | 商用UI・アクセシビリティ | 直近48表示条件等は確認済み。実機と商用状態は未確認。2026-09-14：プレビュー購入導線10画面を axe-core で検査し、サービス情報帯のランドマーク違反、ギフト入力・404のタイトル重複、スキップリンクでフォーカスが本文へ移らない問題を修正（修正後は違反0件、キーボード・フォームエラー・320pxを確認）。[記録](../design/VERIFICATION.md) | 実機Safariの端の挙動・キーボード、PC/スマホ、200%拡大、キーボード操作・読み上げ、空/エラー/処理中/取消/発送/返金を確認。[画面記録](../design/VERIFICATION.md) |
| P0-20 | 監視・復旧・当番 | Smoke/Worker Incident/構造化エラーあり。運用証跡待ち。2026-09-14：Worker Incidentが次の成功実行で閉じ、失敗した通知や要確認の未記録決済が残っていても見えなくなる不具合を修正。毎回未解決件数を数えて解決まで失敗させ、Issueに件数と[障害対応手順](COMMERCE_WORKER_INCIDENTS.md)を載せる（単体試験、DB試験はCIで確認）。同日、失敗した通知を担当者が再処理する保護された手動ワークフローを追加（P1-03）。本文削除済みの通知を解決済みにする手段と、未記録Sessionの取込ツールは未実装 | 決済引継ぎ失敗、署名失敗、DB、Inbox滞留/恒久失敗を検知し担当者へ連絡。再処理、DB復元、切り戻し、取引照合、遅延通知の処理を演習 |
