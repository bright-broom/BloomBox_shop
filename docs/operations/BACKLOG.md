...
| P0-06 | 在庫と決済の実整合 | 予約・確定・解放は実装/DB検証済み。判断・実接続待ち。2026-09-14：決済作成の応答が失われて決済IDを保存できなかった取引を、同じ購入準備IDの署名検証済み期限切れイベントで解放（DB試験追加、CIで確認）。要求自体がStripeに届かなかった取引の照合・復旧は未実装 | 実在庫登録、24時間の予約保持・受付上限の業務確認、同時購入・失敗・取消・期限切れ・通信断・返金を確認。結果不明の取引を期限後に照合して復旧し、タイマーだけで予約を戻さない。[在庫](NATIVE_INVENTORY.md) |
