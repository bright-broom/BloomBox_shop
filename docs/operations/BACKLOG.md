...
| P0-20 | 監視・復旧・当番 | Smoke/Worker Incident/構造化エラーあり。運用証跡待ち。2026-09-14：Worker Incidentが次の成功実行で閉じ、失敗した通知や要確認の未記録決済が残っていても見えなくなる不具合を修正。毎回未解決件数を数えて解決まで失敗させ、Issueに件数と[障害対応手順](COMMERCE_WORKER_INCIDENTS.md)を載せる（単体試験、DB試験はCIで確認）。失敗した通知の再処理ツール（P1-03）と未記録Sessionの取込ツールは未実装 | 決済引継ぎ失敗、署名失敗、DB、Inbox滞留/恒久失敗を検知し担当者へ連絡。再処理、DB復元、切り戻し、取引照合、遅延通知の処理を演習 |
