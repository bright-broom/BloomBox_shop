...
| P0-08 | 税・送料・最終総額・返品条件 | 設定/判断/証跡待ち。[正式APIの金額照合](SHOPIFY_ORDER_PRICING.md)を実装（税込の二重加算防止、商品/送料割引・税額配賦の照合とスナップショット生成、不完全/変更済み注文の保留）。条件付き注文保存Adapterも実装済みだが、注文経路への接続・店舗設定の承認は未完了 | 実事業者の料金と表示を確定し、商品・数量・税・送料・総額・条件が最終確認画面と注文スナップショットで整合 |
