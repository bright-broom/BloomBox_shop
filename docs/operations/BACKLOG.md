...
| P0-08 | 税・送料・最終総額・返品条件 | 税込・日本全国配送、M4,000円＋送料1,000円/L8,000円送料無料は2026-09-12ユーザー確認済み。店舗設定・返品条件・証跡待ち。[正式APIの金額照合](SHOPIFY_ORDER_PRICING.md)を実装（税込の二重加算防止、商品/送料割引・税額配賦の照合とスナップショット生成、不完全/変更済み注文の保留）。条件付き注文保存まで内部接続済みだが、公開購入経路への接続・店舗設定の承認は未完了 | 実事業者の料金と表示を確定し、商品・数量・税・送料・総額・条件が最終確認画面と注文スナップショットで整合 |
...
| P0-20 | 監視・障害検知・復旧演習 | 一部実装。Smoke/Worker Incident/構造化エラーあり。[Shopify Inboxの件数・待機時間・再試行・恒久失敗・保持期限の診断](SHOPIFY_INBOX_DIAGNOSTICS.md)と固定コードによる保留理由別集計を専用閲覧接続のCLIで実装 | 診断の定期実行・通知先接続、決済引継ぎ失敗、署名失敗、未着通知の検知が残件。連絡先・対処・再処理手順、DB復元、ロールバック、取引照合の証跡を残す |
