...
| P1-03 | Inbox/Outboxの再処理運用 | Inbox自動retryとFAILED隔離はある。[復元不能な通知が正常なバッチを止めない再試行](PROVIDER_INBOX_RECOVERY.md)も実装。Outboxは書込のみ | 失敗の調査、権限付きのFAILED再投入、Shopify worker接続・正式API再照合が残件。必要なconsumerを決め、記録だけのOutboxの保持/肥大化方針を定める |
