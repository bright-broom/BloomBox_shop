...
| 在庫の取引処理 | 予約・確定・安全な解放、同時購入、重複、通信断、保存失敗をDBで検証 | 顧客取消は2026-09-14にカート削除へ接続（単体・隔離DB試験）。実Stripe、期限後復旧、発送・返品が残る。[在庫](NATIVE_INVENTORY.md) |
...
| P0-05 | 認証済みの実購入経路 | 内部実装済み・接続/検証待ち。本番はコードで停止。2026-09-14：未決済購入の顧客取消をカート削除へ接続（決済先選択前は即時解放、Session発行済みはStripe失効を確認し検証済み通知で解放）。単体・隔離DB試験のみで実Stripe未確認。カート編集で置き換えた購入準備の取消は未接続。[ADR 0010追補](../architecture/adr/0010-native-inventory-reservations.md) | 本番受付を開けず隔離環境でGoogle→実商品相当DB→予約→Stripeテスト→署名通知→本人注文表示を接続。顧客取消は実Stripeテストで失効要求→期限切れ通知→在庫解放→カート非表示を確認する。[STRIPE](STRIPE.md)、[紐付け](PURCHASE_CUSTOMER_BINDING.md) |
