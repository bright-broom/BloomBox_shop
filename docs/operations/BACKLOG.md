...
| P1-03 | Inbox/Outboxの再処理運用 | Inbox自動retryとFAILED隔離、[復元不能な通知の分離](PROVIDER_INBOX_RECOVERY.md)、[DB所有者による確認・監査付きFAILED再投入](WEBHOOK_RETRY.md)を実装。Outboxは書込のみ | 原因解消・接続設定を伴う復旧演習、Shopify worker接続・正式API再照合が残件。必要なconsumerを決め、記録だけのOutboxの保持/肥大化方針を定める |
