...
- 自作商品の1箱送料を商品DBに登録し、購入開始時に固定してStripe要求・決済通知・注文金額を照合する処理を内部実装。未設定、複数箱、金額不一致を拒否。専用DBとSDKモックで検証済み。実Stripeの最終確認画面・自動税計算・実通知の照合は未実施。[PURCHASE_SHIPPING.md](PURCHASE_SHIPPING.md)参照。
- 自作SKUの在庫予約・確定・解放、在庫表示、未着手の取消・期限切れ、決済通知からの確定・解放を内部実装。同時購入、重複、通信断、保存失敗を専用DBで検証。実在庫登録・調整、取消画面、実Stripeの接続と結果不明の復旧運用は未完了。[NATIVE_INVENTORY.md](NATIVE_INVENTORY.md)参照。本番受付停止は維持。
...
- 顧客マイページは、ユーザー指定によりBloomBox内の独自画面を優先して実装。注文履歴・プロフィール参照・顧客認証・ローカルログアウトの初期実装と、実接続に必要な作業は [CUSTOMER_ACCOUNT.md](CUSTOMER_ACCOUNT.md) に記録。
- 税込、日本全国配送、M＝商品4,000円＋送料1,000円、L＝8,000円・送料無料、紹介特典の最低商品小計5,500円はユーザー確認済み。箱の寸法・花材等の未確定事項や、Shopify上での実注文の金額/配送照合が完了したことを意味しない。
- 公開サイト監視の旧商品検索条件はPR #82で修正・main取込み済み。取込み後の監視成功を確認済み。実店舗の決済・在庫・通知・復旧演習は引き続き別の残件。
- 現在の本番Gateは案内コンテンツと商用有効化の未承認を検出。以下の2026-09-11時点の外部設定スナップショットは、導入時に再確認する。
...
| P2-02 | 顧客アカウント・住所帳・注文履歴 | ユーザー指定でBloomBox内の独自画面を優先。ログイン・本人の注文履歴（10件ずつ）・プロフィール参照・ログアウトを実装。Customer Account APIの実クライアント接続、2顧客での隔離検証、住所/プロフィール編集、注文詳細、紹介特典の本番紐付けは残件。[実装と残件](CUSTOMER_ACCOUNT.md) |
