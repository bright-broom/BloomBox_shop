...
| ID | 課題 | 現在の状態と完了条件 |
| --- | --- | --- |
| P1-01 | 自作注文のサポート検索・操作 | 顧客履歴・Shopify向け担当者画面と、自作注文のサポート経路を区別。最小権限で注文検索・参照/変更監査・PII制限を提供。販売時点で担当者・代替手順は必要 |
...
| P2-06 | 紹介・クーポン・販促・推薦 | 紹介はTest Mode専用。自作顧客と実注文による初回判定、永続台帳、自己/複数/相互紹介対策、実決済の割引、検証済み支払/配達/取消/返金、異議申立て・再処理を接続。推薦は未実装。分析/特典障害で購入を止めない。[REFERRALS](REFERRALS.md) |
