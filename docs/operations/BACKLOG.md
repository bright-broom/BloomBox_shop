...
| P0-05 | 認証済みの実購入経路 | 内部実装済み・接続/検証待ち。本番はコードで停止。2026-09-14：未決済購入の顧客取消をカート削除へ接続（決済先選択前は即時解放、Session発行済みはStripe失効を確認し検証済み通知で解放）。単体・隔離DB試験のみで実Stripe未確認。同日、カートの内容変更・入替の保存でも前の購入準備を先に取り消すよう接続（単体試験）。終了済み・支払手続き完了済みの購入準備に紐付いたカートは、想定外エラーではなく要求IDの更新またはカートを空にして案内する（単体試験）。[ADR 0010追補](../architecture/adr/0010-native-inventory-reservations.md) | 本番受付を開けず隔離環境でGoogle→実商品相当DB→予約→Stripeテスト→署名通知→本人注文表示を接続。顧客取消は実Stripeテストで失効要求→期限切れ通知→在庫解放→カート非表示を確認する。[STRIPE](STRIPE.md)、[紐付け](PURCHASE_CUSTOMER_BINDING.md) |
