...
Rollout step 3 now has disabled-by-default signed webhook capture and provider/account-scoped encrypted Inbox storage. It stores minimal order/refund references, deduplicates by body digest rather than unsigned headers, and does not project commerce state. A disconnected Admin API reader now validates the pinned shop, reference identity, complete bounded line/transaction collections, JPY money, and current order/refund facts. These facts still require a trustworthy association with the persisted checkout attempt before projection; provider attributes alone do not authorize that association. See [read boundary](../../operations/SHOPIFY_ORDER_READS.md). Processing, reconciliation, retention completion, and real-store evidence remain pending. See [webhook capture](../../operations/SHOPIFY_WEBHOOKS.md).
