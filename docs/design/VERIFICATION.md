...
ユーザーの配色改善依頼により、主操作・現在位置・フォーカスをローズプラムへ変更。変更レベルはL1。CSSの `--accent`（#785568）と `--accent-hover`（#634353）のみを変更し、全画面で共有する。成功・エラーの意味色と購入処理は変更していない。

- sRGB相対輝度から計算した白とのコントラスト：通常6.37:1、hover 8.54:1。通常色と補助面は5.84:1、入力面は5.94:1。
- ローカルで `pnpm check:ci` 成功：型・lint・デザイン規約・境界検査、245件のテスト、本番ビルド。既存DB7件はローカル設定なしでskip。
- 開発サーバーで1280pxのホームと390pxのギフト入力を目視確認。390pxでは文書幅・スクロール幅が一致し、横overflowなし。
- Tabで主ボタンへ移動し、白文字、ローズプラムの背景と2pxのフォーカス輪郭を確認。hoverはトークンと既存CSSルール、コントラスト計算を確認。実ポインターのhover・スクリーンリーダーの再試験は行っていない。
