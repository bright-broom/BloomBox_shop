...
Version 2.0 · 2026-09-11 · 対象：公開サイト、商品一覧・詳細、ギフト、購入手続き、案内・法務ページ
...
採用するのはメインサイトの白・黒・青のプロファイル。LP用の緑色CTAと混在させない。

| 参照の特徴 | BloomBoxでの実装 | 意図的な調整 |
| --- | --- | --- |
| コンパクトなヘッダー、右側の導線 | PC 62px、モバイル54px、青いCTA | ブランドはBloomBox。プレビュー表示を別の28px帯で保持 |
...
2. **一つの領域に主操作は一つ。** 青は購入・次へ進む操作に使う。戻る・詳しく読む・絞り込みは副操作。
...
| `--accent` | `#0171E3` | 主操作、現在位置、フォーカス |
| `--accent-hover` | `#005CBF` | 主操作のhover |
...
白に対する`--accent`は約4.70:1、`--ink-soft`は約5.60:1。薄い`--line`を入力の唯一の識別手段として使わない。色値は`:root`内に置き、コンポーネントでは役割名で参照する。
...
| PrimaryAction | `.primary-button` | 青地・白文字。hoverは濃い青。disabledは操作不可と透明度。送信中の文言は呼び出し側 |
...
本文へ移動するスキップリンク、見出し階層、明示的なラベル、キーボードフォーカスを維持する。白い面のfocusは青、黒い面とヒーローでは白い輪郭を使う。入力エラーは枠色と文章の両方で表示する。
...
[ホーム](evidence/home-desktop.jpg)、[商品カード](evidence/collection-desktop.jpg)、[モバイルの入力エラー](evidence/form-error-mobile.jpg)を参照。各画像は実装から取得した表示例であり、トークンやレスポンシブ規約の正本は上記のCSSと表。
