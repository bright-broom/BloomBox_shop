...
参照サイトの白・黒と余白の構成を土台に、BloomBoxの主操作色はローズプラムへ調整する（2026-09-11、ユーザーのアクセントカラー改善依頼）。花弁を思わせる低彩度の赤紫を、写真に添える小さな色として使う。包装の最終配色を確定するものではない。

| 参照の特徴 | BloomBoxでの実装 | 意図的な調整 |
| --- | --- | --- |
| コンパクトなヘッダー、右側の導線 | PC 62px、モバイル54px、ローズプラムのCTA | ブランドはBloomBox。プレビュー表示を別の28px帯で保持 |
...
2. **一つの領域に主操作は一つ。** ローズプラムは購入・次へ進む操作に使う。戻る・詳しく読む・絞り込みは副操作。
...
| `--accent` | `#785568` | 主操作、現在位置、フォーカス（ローズプラム） |
| `--accent-hover` | `#634353` | 主操作のhover |
...
白に対する`--accent`は約6.37:1、`--accent-hover`は約8.54:1。補助面`--surface`に対する`--accent`は約5.84:1、入力面`--field`では約5.94:1、`--ink-soft`は約5.60:1。薄い`--line`を入力の唯一の識別手段として使わない。色値は`:root`内に置き、コンポーネントでは役割名で参照する。
...
| PrimaryAction | `.primary-button` | ローズプラム地・白文字。hoverは深いプラム。disabledは操作不可と透明度。送信中の文言は呼び出し側 |
...
本文へ移動するスキップリンク、見出し階層、明示的なラベル、キーボードフォーカスを維持する。白い面のfocusはローズプラム、黒い面とヒーローでは白い輪郭を使う。入力エラーは枠色と文章の両方で表示する。
